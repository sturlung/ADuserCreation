...
$result = Import-Csv -Path ".\inputfile.csv" -Delimiter ";"
  ...
  for ($i=1; $i -le 8; $i++){
    ...
    $type = Get-Random -Minimum 1 -Maximum 4
        ...
        switch ($type){
            ...
            1 {$chara = Get-Random -Minimum 0 -Maximum 10
            $password = $password + $numbers[$chara]
            $n++}

            2 {$chara = Get-Random -Minimum 0 -Maximum 26
            $password = $password + $capitals[$chara]
            $c++}
            
            3 {$chara = Get-Random -Minimum 0 -Maximum 26
...
if ($expirationmidnight -eq $null){ $expirationmidnight = $today.AddYears(1)} #to :)
